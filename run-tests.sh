#!/usr/bin/env bash
# Build + run the cajeta-docs unit tests.
#
...
# an executable, with the docs library and cajeta-unit supplied as .cja
...
echo ">> building docs library .cja"
"$CAJETA" --emit=cja -o "$out/docs.cja" \
    dev.cajeta.docs.Docs.run "$here/src/main/cajeta" "$out" >/dev/null

echo ">> building + running the test binary"
"$CAJETA" --emit=exe --profile=test \
    --classpath="$out/docs.cja,$unit_cja" \
    -o "$out/dtests" \
    dev.cajeta.docs.selftest.TestMain.run "$here/src/test/cajeta" "$out" >/dev/null

# Parity tests load sklearn-1.9.0-pinned golden fixtures from
# tools/fixtures via this env var (committed .npy, gen_docs.py).
export DOCS_FIXTURES="$here/tools/fixtures"
"$out/dtests" \
    dev.cajeta.docs.selftest.TestMain.run "$here/src/test/cajeta" "$out" >/dev/null
