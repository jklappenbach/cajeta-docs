...
echo ">> building dev.cajeta.docs"
"$CAJETA" build >/dev/null
art="$(ls -t "$here"/build/archive/dev.cajeta.docs-*.cja | head -1)"

echo ">> compiling the tour"
mkdir -p build/tour
"$CAJETA" --emit=exe --classpath="$art" \
    -o build/tour/docs-tour \
    dev.cajeta.docs.tour.Tour.main "$here/tour/src" build/tour >/dev/null

echo ">> running"
exec ./build/tour/docs-tour
